...
- Runtime 只接受 manifest 固定的 FEAT-126 日志安全 patch → Owner 授权 FEAT-136 early sandbox-denial lifecycle patch → Owner 授权 FEAT-137 stable sandbox provenance patch → Owner 授权 FEAT-137 deterministic approval producer patch 的精确顺序；第四个 patch 默认关闭，仅可由 exact D4 Host gate 在完整 approval authority 通过后注入 Runtime 私有开关；canonical `yijie-codex/codex-rs` 仍不得直接修改；
- exact FEAT-137 approval authority 的受管 MiniMax profile 必须逐项关闭 hooks、plugins、apps、tool suggestion 与 shell snapshot，且不得声明 MCP server 或 dynamic tool；provider request/stream retries 必须同时固定为 0；D4 缺少该 exact 配置时在启动 Runtime 前失败；approval 与 gate-off MiniMax profile 都在 spawn 前重验固定 model catalog，gate-off 配置保持既有字节；
