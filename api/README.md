...
Run `YIJIE_CONTRACTS_REF=<full-contracts-commit> make sync-contracts` from a clean checkout with sibling `yijie-contracts`; the current FEAT-127 local candidate uses the full commit recorded in `contracts.lock`. The sync reads contract sources from that Git object, not from floating worktree files. A `contracts-vX.Y.Z` tag is used only after an approved supported release exists, so the untagged `0.3.0` candidate must not be described as published. Normal Host CI runs `make contract-check` without requiring network access. Do not edit snapshot or generated files by hand.
